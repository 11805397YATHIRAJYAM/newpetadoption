...
// Catch startup crashes and log the real error
...
if (process.env.NODE_ENV === 'production') {
  ...
  // Unknown /api routes → JSON 404 (must be before the * wildcard)
  app.use('/api', notFound);

  // All other routes → React Router (SPA)
  app.get('*', (req, res) => {
    res.sendFile(path.join(clientDist, 'index.html'));
  });
} else {
  app.get('/', (req, res) => {
    res.json({ success: true, message: 'Pet Adoption API', version: '1.0.0' });
  });
  app.use(notFound);
}
...
server.listen(PORT, () => {
  ...
  if (process.env.NODE_ENV === 'production' && process.env.RENDER_EXTERNAL_URL) {
    setInterval(() => {
      ...
      https.get(url, () => console.log('Keep-alive ping sent')).on('error', () => {});
    }, 14 * 60 * 1000);
  }
});
